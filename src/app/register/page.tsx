        ...
        <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/20 dark:border-slate-800 rounded-3xl p-8 shadow-2xl">
          ...
          <form action={formAction} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-900 dark:text-slate-200 mb-1.5" htmlFor="roll_number">Roll Number</label>
                <input id="roll_number" name="roll_number" placeholder="e.g. 21CS001" className="w-full px-4 py-2.5 rounded-xl bg-slate-100/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-1">
                ...
                <input id="year" name="year" type="number" min="1" max="4" required defaultValue="2" className="w-full px-4 py-2.5 rounded-xl bg-slate-100/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white" />
              </div>
              <div className="col-span-1">
                <label className="block text-sm font-medium text-slate-900 dark:text-slate-200 mb-1.5" htmlFor="semester">Semester</label>
                <input id="semester" name="semester" type="number" min="1" max="8" required defaultValue="4" className="w-full px-4 py-2.5 rounded-xl bg-slate-100/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white" />
              </div>
              <div className="col-span-1">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-900 dark:text-slate-200 mb-1.5" htmlFor="department">Department</label>
              <input id="department" name="department" type="text" required placeholder="e.g. Computer Science" className="w-full px-4 py-2.5 rounded-xl bg-slate-100/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white uppercase" />
            </div>

            <button
              type="submit"
              disabled={isPending}
              className="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white rounded-xl px-4 py-3 font-semibold shadow-sm hover:bg-indigo-500 hover:shadow-indigo-500/25 transition-all mt-6 disabled:opacity-50"
            >
              {isPending ? 'Creating...' : <>Create Account <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>
